...
data_buffer = []
LAST_SESSION = []
MAX_ANGLES = {"hipLT": 0.0, "hipRT": 0.0, "kneeLT": 0.0, "kneeRT": 0.0, "ankleLT": 0.0, "ankleRT": 0.0}
...
_SMOOTH_STATE = {"hipLT": 0.0, "kneeLT": 0.0, "ankleLT": 0.0}
# Giảm alpha xuống 0.05 để chống rung "lật" quá nhanh khi mạch đo với mẫu lên đến 100Hz
_SMOOTH_ALPHA = {"hipLT": 0.05, "kneeLT": 0.05, "ankleLT": 0.05}
...
def quaternion_to_euler(w: float, x: float, y: float, z: float):
    """
    Hàm chuyển đổi Quaternion sang Euler đã được sửa lỗi kẹt ở 90 độ.
    Đảm bảo trục PITCH có thể chạy liên tục từ -180 đến 180 độ.
    """
    sinp = 2 * (w * y - z * x)
    pitch = math.atan2(2 * (w * y - z * x), w * w + x * x - y * y - z * z)
    sinr_cosp = 2 * (w * x + y * z)
...
def compute_relative_quaternion(q_sensor, q_reference):
    ...
    Lưu ý: Dựa trên công thức Table 1: q_relative = q_{parent}^{-1} * q_{child}
    (Ví dụ Trunk: q_{LB}^{TR} = q_{LB}^E * q_E^{TR})
    Khi đó sensor là phần ngoài (child), reference là phần gốc (parent)
    """
    q_ref_inv = quaternion_inverse(q_reference)
    return quaternion_multiply(q_ref_inv, q_sensor)
...
def reset_max_angles():
    with MAX_LOCK:
        MAX_ANGLES["hipLT"] = 0.0
        MAX_ANGLES["hipRT"] = 0.0
        MAX_ANGLES["kneeLT"] = 0.0
        MAX_ANGLES["kneeRT"] = 0.0
        MAX_ANGLES["ankleLT"] = 0.0
        MAX_ANGLES["ankleRT"] = 0.0
        ...
        while not stop_serial_thread:
            try:
                ...
                if ptype == "imu":
                    ...
                    # ----- PHẦN CHI DƯỚI (PHÂN GIẢI TRỌNG TRƯỜNG TUYỆT ĐỐI) -----
                    # LƯU Ý KỸ THUẬT QUAN TRỌNG VỀ IMU:
                    # Việc dùng Quaternion tương đối (q_inv * q) theo chuẩn ISB CHỈ KHẢ THI khi hệ thống có bước 'TARE/CALIBRATE'.
                    # Nếu các cảm biến chưa chung hệ tham chiếu Heading (Bắc từ/Yaw offset)v, Quaternion tương đối sẽ gây ra 
                    # hiện tượng "bẻ chéo chân" (Trộn trục Z/Y) và "lật ngược quỹ đạo".
                    # Vì thế, thuật toán TỐT NHẤT ở đây là dùng Inclination Matching (Tham chiếu theo Trọng lực Trái đất):
                    p1 = last_angles.get(1, {}).get("pitch", 0.0)
                    p2 = last_angles.get(2, {}).get("pitch", 0.0)
                    p3 = last_angles.get(3, {}).get("pitch", 0.0) 
                    p4 = last_angles.get(4, {}).get("pitch", 0.0)

                    # Bù trừ trên hệ Euler Pitch gốc (Kháng hoàn toàn nhiễu Heading Yaw):
                    raw_hip = norm_deg(p1 - p2 )
                    raw_knee = norm_deg(p3 - p2)
                    raw_ankle = norm_deg((p4 - p3) - 90.0)
                    
                    hip_val = raw_hip
                    if abs(hip_val) < DEADZONE:
                         hip_val = 0.0

                    hipLT = _smooth("hipLT", clamp(hip_val, -10.0, 130.0)) 
                    kneeLT = _smooth("kneeLT", clamp(raw_knee, -10.0, 130.0))
                    ankleLT = _smooth("ankleLT", clamp(raw_ankle, -20.0, 40.0))

                    # ----- PHẦN CHI TRÊN (Đọc Quaternion gốc từ map) -----
                    # Giả sử thiết lập ID: LowerBack(4), Trunk(5), Arm(6), Forearm(7), Hand(8)
                    q_lowerback=(1.0, 0.0, 0.0, 0.0)
                    q_trunk   = last_angles.get(1, {}).get("q", (1.0, 0.0, 0.0, 0.0))
                    q_arm     = last_angles.get(2, {}).get("q", (1.0, 0.0, 0.0, 0.0))
                    q_forearm = last_angles.get(6, {}).get("q", (1.0, 0.0, 0.0, 0.0))
                    q_hand    = last_angles.get(7, {}).get("q", (1.0, 0.0, 0.0, 0.0))
                    ...
                    # ----- PHẦN CHI TRÊN (Đã vô hiệu hóa tính toán) -----
                    # # Gán tất cả giá trị góc bằng 0 để giữ cấu trúc gửi lên giao diện UI
                    # theta1 = theta2 = theta3 = 0.0
                    # theta4 = theta5 = theta6 = 0.0
                    # theta7 = theta8 = 0.0
                    with MAX_LOCK:
                        if hipLT > MAX_ANGLES["hipLT"]: MAX_ANGLES["hipLT"] = hipLT
                        if kneeLT > MAX_ANGLES["kneeLT"]: MAX_ANGLES["kneeLT"] = kneeLT
                        if ankleLT > MAX_ANGLES["ankleLT"]: MAX_ANGLES["ankleLT"] = ankleLT
                        max_payload = {
                            "maxHip": MAX_ANGLES["hipLT"],
                            "maxKnee": MAX_ANGLES["kneeLT"],
                            "maxAnkle": MAX_ANGLES["ankleLT"],
                        }

                    with DATA_LOCK:
                        data_buffer.append({
                            "t_ms": now_ms, "hipLT": hipLT, "kneeLT": kneeLT, "ankleLT": ankleLT,
                            "upper_body": {
                    ...
                    if now_sys - getattr(reader_loop, "last_emit", 0) > 0.033:
                        socketio.emit("imu_data", {
                            "t": now_ms, "hipLT": hipLT, "kneeLT": kneeLT, "ankleLT": ankleLT, 
                            "upper_body": {
                                "trunk": (theta1, theta2, theta3),
                                "arm": (theta4, theta5, theta6),
                                "forearm": (theta7, theta8),
