import os, json, time, math, io, csv, threading, sys
...
def resource_path(relative_path):
    """ Lấy đường dẫn tuyệt đối, hoạt động cho cả khi chạy code thường và file .exe """
    try:
...
# --- Thêm đoạn này vào webgiaodien.py ---
@app.route('/patients/new', methods=['GET', 'POST'])
# @login_required  <-- Nếu code của bạn có dùng login thì bỏ comment dòng này ra
def view_patient_new():
    if request.method == 'POST':
        full_name = request.form.get('full_name', '').strip()
        national_id = request.form.get('national_id', '').strip()
        dob = request.form.get('dob', '').strip()
        sex_raw = request.form.get('sex', '').strip()
        ...
        sex = "Male" if sex_raw == "Male" else "FeMale" if sex_raw == "Female" else ""

        # Load và lưu dữ liệu
        _, raw_data = load_patients_rows()
        code = f"BN{int(time.time())}"

        raw_data[code] = {
            "PatientCode": code, "name": full_name, "ID": national_id,
            "DateOfBirth": dob, "Gender": sex, "Weight": weight, "Height": height,
            "Exercise": {}
        }
        ...
        try:
            ...
            flash(f'Đã thêm: {full_name}', 'success')
            return redirect('/')
        except Exception as e:
            flash(f'Lỗi lưu file: {str(e)}', 'danger')
...
# --- API LẤY DANH SÁCH BỆNH NHÂN (GET) ---
@app.route('/api/patients', methods=['GET'])
def get_patients_api():
    rows, raw = load_patients_rows()  # Hàm này bạn đã có ở code cũ
    return jsonify({"rows": rows, "raw": raw})


# --- API LƯU BỆNH NHÂN (POST) ---
# Hàm này dùng chung cho cả Thêm mới và Cập nhật
...
def save_patient_api():
    try:
        ...
        if not name:
            return jsonify({"ok": False, "msg": "Thiếu tên bệnh nhân"})

        patients = load_patients()  # Hàm load_patients() trả về dict, bạn cần đảm bảo hàm này có sẵn
        # Nếu dùng load_patients_rows() trả về (rows, raw) thì dùng: _, patients = load_patients_rows()

        code = data.get('patient_code', '').strip()

        # Nếu không có code -> Tạo mới
        if not code:
            code = f"BN{int(time.time())}"
        ...
        patients[code] = {
            ...
            "Exercise": patients.get(code, {}).get("Exercise", {})  # Giữ lại dữ liệu bài tập cũ nếu có
        }
        ...
        return jsonify({"ok": True, "patient_code": code, "msg": "Lưu thành công"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})


# --- API XÓA 1 BỆNH NHÂN (DELETE) ---
@app.route('/api/patients/<code_id>', methods=['DELETE'])
def delete_patient_api(code_id):
    try:
        _, patients = load_patients_rows()
        if code_id in patients:
            del patients[code_id]
            with open(PATIENTS_FILE, "w", encoding="utf-8") as f:
                json.dump(patients, f, ensure_ascii=False, indent=2)
            return jsonify({"ok": True, "msg": "Đã xóa"})
        else:
            return jsonify({"ok": False, "msg": "Không tìm thấy mã bệnh nhân"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})


# --- API XÓA TOÀN BỘ (DELETE) ---
@app.route('/api/patients', methods=['DELETE'])
def delete_all_patients_api():
    try:
        # Ghi đè file bằng dict rỗng
        with open(PATIENTS_FILE, "w", encoding="utf-8") as f:
            json.dump({}, f, ensure_ascii=False, indent=2)
        return jsonify({"ok": True, "msg": "Đã xóa toàn bộ dữ liệu"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})
@app.route("/records")
@login_required
...
const GLB_URL = "{{ url_for('static', filename='models/fullbody3.glb') }}";

loader.load(
  GLB_URL,
  (gltf) => {
    const model = gltf.scene || gltf.scenes?.[0];
    if (!model) { statusEl.textContent = " GLB không có scene."; return; }
...
if __name__ == "__main__":
    ...
    # Đợi 1.5 giây rồi mở web
    Timer(1.5, open_browser).start()

    # CHÚ Ý: Bắt buộc để debug=False thì file EXE mới chạy ổn định và tự mở web được
